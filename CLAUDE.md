...
```bash
sshpass -p "$CERBO_ROOT_PASSWORD" ssh root@venus.local 'tail -100 /var/log/fla-equalisation/current'
sshpass -p "$CERBO_ROOT_PASSWORD" ssh root@venus.local 'tail -100 /var/log/fla-charge/current'
```
