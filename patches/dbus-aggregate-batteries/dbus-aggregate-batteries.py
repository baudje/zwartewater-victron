...
class DbusAggBatService(object):
    ...
    def _update(self):
        ...
        if settings.CURRENT_FROM_VICTRON:
            ...
            if success:
                # [Zwartewater patch] When a battery-mode SmartShunt is configured to be
                # the authoritative bank-current source, use Current_SHUNTS directly and
                # ignore the Quattro+MPPT sum (which would otherwise double-count, since
                # the shunt already includes their contributions to the bank).
