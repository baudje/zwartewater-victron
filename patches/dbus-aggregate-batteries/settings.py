...
# [Zwartewater patch] Parse USE_SMARTSHUNTS as bool OR list-of-ints/strings.
# Upstream reads it as bool only, so `USE_SMARTSHUNTS = [277]` silently becomes False.
def get_smartshunts_from_config(group: str, option: str):
    """
    Parse USE_SMARTSHUNTS as a bool or as a list literal.

    Accepts:
      - "True"/"False" (bool)
      - "[277]" or "[277, 279]" (list of ints)
      - "['MyShunt', 279]" or '["MyShunt", 279]' (list mixing names/ids)
      - empty (= False)

    Returns bool or list. Uses JSON for list parsing (no code execution).
    """
    import json
    ...
    if raw.startswith("[") and raw.endswith("]"):
        ...
        if isinstance(val, list):
            # Each element must be an int (VRM instance) or str (custom name).
            # The driver's match loop only handles those two types; bool/float/None
            ...
            if bad:
                errors_in_config.append(
                    f"Invalid USE_SMARTSHUNTS list elements: {bad!r}. "
                    "Each entry must be an int (VRM instance) or str (custom name)."
                )
                return False
            return val
    errors_in_config.append(
        f"Invalid USE_SMARTSHUNTS value: {raw!r}. Expected True/False or list like [277]."
    )
    return False
...
# [Zwartewater patch] Use list-aware parser; also expose new SMARTSHUNT_AS_BATTERY_CURRENT.
...
# When True, a battery-mode SmartShunt's reading is used as the authoritative
# bank current (skipping the Quattro+MPPT addition that would otherwise double-count).
